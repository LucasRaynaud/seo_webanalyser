          ...
          <div className="results-overview">
            ...
            <div className="pages-table-container">
              ...
              <table className="pages-table">
                ...
                <tbody>
                  {analysisResults.results.map((page, index) => (
                    <tr key={index} className={page.error ? 'error-row' : ''}>
                      ...
                      <td className={page.missingH1 || page.hasMultipleH1 ? 'issue-cell' : ''}>
                        {page.missingH1 ? (
                          ...
                          <span className="issue-data">{page.h1Count} H1</span>
                        ) : (
                          <span title={page.h1Text}>
                            {page.h1Text.length > 30 ? page.h1Text.substring(0, 27) + '...' : page.h1Text}
                          </span>
                        )}
                      </td>
                      <td>{page.h2Count || 0}</td>
                      {analysisType === 'full' && (
                        <>
                          <td className={page.loadTime > 3 ? 'issue-cell' : ''}>
                            {page.loadTime ? `${page.loadTime.toFixed(2)}s` : '-'}
                          </td>
                          <td className={page.fcp > 2.5 ? 'issue-cell' : ''}>
...
function exportToCSV(analysisResults) {
  ...
  const hasPerformanceMetrics = results.some(r => r.fcp !== undefined);
  ...
  for (const page of results) {
    const row = [
      ...
      page.metaTitleLength || 0,
      `"${(page.metaDescription || '').replace(/"/g, '""')}"`,
      page.metaDescriptionLength || 0,
      page.h1Count || 0,
      `"${(page.h1Text || '').replace(/"/g, '""')}"`,
      page.h2Count || 0
    ];
    
    if (hasPerformanceMetrics) {
      row.push(
        page.loadTime ? page.loadTime.toFixed(2) : '',
        page.fcp ? page.fcp.toFixed(2) : '',
        page.seoScore !== undefined ? Math.round(page.seoScore) : ''
      );
    }
